(() => {
  ...
  const socialStateModal = bootstrap.Modal.getOrCreateInstance(
    $("#socialStateModal"),
  );
  ...
  function updateCsrfToken(token) {
    ...
    /*
     * Actualiza tanto el formulario principal como
     * el modal de redes sociales.
     */
    document
      .querySelectorAll('#settingsModule input[name="csrf_token"]')
      .forEach((input) => {
  ...
  function renderImageCurrent(selector, path, label, compact = false) {
    ...
    if (!path) {
      container.innerHTML =
        '<span class="text-body-secondary small">Sin archivo actual</span>';
      return;
    }
  ...
  function renderSocials(socials) {
    ...
    socialTableBody.innerHTML = socials
      .map((social) => {
  ...
  async function loadSettings() {
    try {
      ...
      $("#totalSettings").textContent =
        data.resumen?.total_configuraciones ?? 0;
  ...
  function resetFileInputs() {
    ["#profilePhoto", "#resumeFile", "#seoImage", "#faviconFile"].forEach(
      (selector) => {
        $(selector).value = "";
      },
    );
  ...
  async function saveSocial(event) {
    ...
    try {
      ...
      setButtonLoading(
        button,
        false,
        $("#socialId").value === "0" ? "Guardar red social" : "Guardar cambios",
      );
    }
  }
  ...
  function askSocialState(button) {
    ...
    const action = pendingSocialState.active === 1 ? "activar" : "dar de baja";
  ...
  function updateSocialIconPreview() {
    ...
    $("#socialIconPreview").innerHTML =
      `<i class="${escapeHtml(className)}"></i>`;
  }
