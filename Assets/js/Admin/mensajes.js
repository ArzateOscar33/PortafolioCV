(() => {
    ...
    const stateModal = bootstrap.Modal.getOrCreateInstance(
        stateElement
    );
    ...
    function stateLabel(state) {
        const item = states.find(
            (candidate) => candidate.slug === state
        );
        ...
        return String(state || 'Sin estado')
            .replaceAll('_', ' ');
    }
    ...
    function renderMessages(messages) {
        ...
        tableBody.innerHTML = messages.map((message) => {
            ...
            const appointment = Number(message.tiene_cita) === 1
                ? `
                    <span
    ...
    async function loadMessages() {
        ...
        try {
            ...
            $('#newMessages').textContent =
                data.resumen?.nuevas ?? 0;

            $('#trackingMessages').textContent =
                data.resumen?.seguimiento ?? 0;

            $('#answeredMessages').textContent =
                data.resumen?.respondidas ?? 0;
    ...
    function buildWhatsapp(phone, message) {
        ...
        }, te contacto respecto a tu solicitud: ${
            message.asunto || 'Proyecto'
        }.`;
    ...
    function renderDetail(message) {
        $('#messageDetailId').value =
            message.id_solicitud_contacto || 0;

        $('#messageDetailTitle').textContent =
            `Mensaje #${message.id_solicitud_contacto}`;

        $('#messageDetailSubject').textContent =
            message.asunto || 'Sin asunto';

        $('#messageDetailState').innerHTML =
            stateBadge(message.estado);

        $('#messageDetailBody').textContent =
            message.mensaje || '';

        $('#messageDetailName').textContent =
            message.nombre_completo || 'Sin nombre';

        $('#messageDetailEmail').textContent =
            message.correo || 'Sin correo';

        $('#messageDetailPhone').textContent =
            message.telefono || 'Sin teléfono';
        ...
        $('#messageDetailPreferredDate').textContent =
            message.fecha_preferida
                ? formatDate(
                    `${message.fecha_preferida}T00:00:00`,
                    false
                )
                : 'Sin fecha preferida';

        $('#messageDetailMode').textContent =
            modalityLabel(message.modalidad_preferida);

        $('#messageDetailLinkedContact').textContent =
            linkedContactText(message);

        $('#messageDetailOrigin').textContent =
            originLabel(message.origen);

        $('#messageDetailIp').textContent =
            message.direccion_ip || 'No registrada';

        $('#messageDetailCreated').textContent =
            formatDate(message.creado_en);

        $('#messageDetailAppointment').textContent =
            message.id_cita
                ? `Cita #${message.id_cita} · ${
                    formatDate(message.cita_inicia_en)
                }`
                : 'Sin cita vinculada';

        $('#btnReplyMessage').href =
            buildMailto(message);

        const whatsappUrl = buildWhatsapp(
            message.telefono,
            message
        );
    ...
    async function markRead(message) {
        ...
        body.append(
            'csrf_token',
            $('#messageStateForm [name="csrf_token"]').value
        );
    ...
    async function saveState(event) {
        ...
        try {
            ...
            if (
                currentMessage
                && Number(
                    currentMessage.id_solicitud_contacto
                ) === id
            ) {
    ...
    function openContactModal() {
        ...
        $('#messageContactMessageId').value =
            currentMessage.id_solicitud_contacto;

        $('#messageContact').value =
            currentMessage.id_contacto_cliente || '0';
    ...
    tableBody.addEventListener('click', (event) => {
        ...
        if (button.dataset.action === 'state') {
            openStateModal(
                id,
                button.dataset.state || 'leida'
            );
        }
    });

    $('#messageStateForm').addEventListener(
        'submit',
        saveState
    );

    $('#messageContactForm').addEventListener(
        'submit',
        saveContact
    );

    $('#btnRefreshMessages').addEventListener(
        'click',
        loadMessages
    );

    $('#btnClearMessageFilters').addEventListener(
        'click',
        clearFilters
    );

    $('#btnChangeMessageState').addEventListener(
        'click',
        () => {
            if (!currentMessage) {
                return;
            ...
            openStateModal(
                currentMessage.id_solicitud_contacto,
                currentMessage.estado
            );
        }
    );

    $('#btnLinkMessageContact').addEventListener(
        'click',
        openContactModal
    );

    $('#btnCopyMessage').addEventListener(
        'click',
        copyMessage
    );
