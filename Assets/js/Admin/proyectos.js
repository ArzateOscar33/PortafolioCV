(() => {
    ...
    function renderCatalogs() {
        ...
        stateFilter.innerHTML = '<option value="0">Todos</option>'
            + catalogs.estados.map((state) => option(
                state.id_estado_proyecto,
                state.nombre
            )).join('');

        projectState.innerHTML = catalogs.estados.map((state) => option(
            state.id_estado_proyecto,
            state.nombre,
            state.slug === 'borrador'
        )).join('');

        projectClient.innerHTML = '<option value="0">Proyecto personal / sin cliente</option>'
            + catalogs.clientes.map((client) => option(
                client.id_cliente,
                client.nombre_mostrar
            )).join('');

        categories.innerHTML = catalogs.categorias.map((category) => option(
            category.id_categoria,
            `${category.nombre}${Number(category.activa) === 1 ? '' : ' (Inactiva)'}`
        )).join('');

        technologies.innerHTML = catalogs.tecnologias.map((technology) => option(
    ...
    function updateSummaryCounter() {
        $('#projectSummaryCounter').textContent = `${$('#projectSummary').value.length} / 350`;
    }
